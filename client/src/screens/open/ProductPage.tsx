...
export function ProductPage () {
    ...
    const [ reviewForm, setReviewForm ] = useState<{ title: string, comment: string, user: string, rating: number }>({
        title: "",
        comment: "",
        rating: 0,
        user: "",
    ...
    const { currentData: fetchedProduct, isLoading: loadingProduct, error: productError } = useGetProductByIdQuery(productId!, {
        skip: !productId,
        refetchOnMountOrArgChange: true,
    });
    const { currentData: fetchedCategoryProducts } = useGetCategoryProductsByIdQuery(fetchedProduct?.data?.category._id, {
    ...
    const handleRatingSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            setReviewForm({ title: "", comment: "", rating: 0, user: "" });
        } catch (error: any) {
