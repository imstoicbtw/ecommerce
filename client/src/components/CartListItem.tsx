        ...
        >
            <Link to={`/product/${product._id}`}>
                <img
                    ...
                    className={"size-50 object-cover rounded-lg border-2 border-blue-100"}
                />
            </Link>
            <div className={"flex flex-col gap-1 h-full p-3 justify-center"}>
                <Button
                    variant={"destructive"}
                    className={"absolute !p-1.5 !rounded-full right-3 top-3"}
                    size={"large"}
                    onClick={handleRemoveFromCart}
                ><TrashIcon /></Button>
                <Link to={`/product/${product._id}`} className={"link"}>
                    <h2 className={"font-semibold text-xl flex gap-1 items-center"}>
                        {product.name}
                ...
                <div className={"mt-1"}>
                    {!product.onSale
                        ? <p className={"text-3xl font-bold"}>
                            <span className={"font-light text-3xl mr-1"}>₹</span>
                            <span>{product.price}</span>
                            <span className={"font-light text-3xl"}>/-</span>
                        </p>
                        : <div>
                            <p>
                                <span className={"text-green-600 font-bold mr-3 text-3xl"}>
