...
export const CartListItem = ({ cartItem }: Props) => {
    ...
    return (
        <li
            className={"relative grid grid-cols-4 items-center md:gap-3 bg-blue-50 p-1 sm:p-3 rounded-xl sm:rounded-2xl border-2 border-blue-100"}
        ...
        >
            <Link to={`/product/${product._id}`} className={"col-span-1"}>
                <img
                    ...
                    className={"aspect-square object-cover rounded-lg border-2 border-blue-100"}
                />
            </Link>
            <div className={"flex flex-col col-span-3 sm:gap-1 h-full ml-2 sm:ml-0 sm:p-3 justify-center"}>
                <Button
                    variant={"destructive"}
                ...
                <div className={"sm:mt-1"}>
                    {!product.onSale
                        ? <p className={"text-lg sm:text-3xl font-bold"}>
                            <span className={"font-light text-lg sm:text-3xl mr-1"}>$</span>
                            <span>{product.price}</span>
                            <span className={"font-light text-lg sm:text-3xl"}>/-</span>
                        </p>
                        : <div>
                            <p>
                                <span className={"text-green-600 font-bold mr-2 sm:mr-3 text-lg sm:text-3xl"}>
                                    <span className={"font-light mr-1"}>$</span>
                                    <span>{product.salePrice}</span>
                                    <span className={"font-light"}>/-</span>
                                </span>
                                <span className={"text-slate-500 font-bold sm:text-2xl"}>
                                    <span className={"font-light"}>$</span>
                            ...
                            <p className={"text-xs sm:text-sm"}>
                                    <span className={"py-0.5 px-1.5 rounded-full bg-blue-500 text-white font-semibold"}>
                                        {Math.round((product.salePrice! / product.price) * 100 - 100)}%
                                    </span>
