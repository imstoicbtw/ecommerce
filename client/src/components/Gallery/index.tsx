...
type Props = {
    gallery: Array<IMediaRawDoc & { _id: string } | null>;
}

export default function Gallery ({ gallery }: Props) {
    const location = useLocation();
    const [ activeImage, setActiveImage ] = useState<IMediaRawDoc & { _id: string } | null>(gallery[0]);
    ...
    return (
        <div className={"flex gap-3 flex-row-reverse justify-between align-start aspect-square "}>
            <div className={"basis-[95%] h-full grow"}>
                <img src={activeImage?.url || "/placeholder.png"} alt={activeImage?.name} className={"w-full h-full border border-slate-300 object-contain rounded-xl sm:rounded-3xl"} />
            </div>
